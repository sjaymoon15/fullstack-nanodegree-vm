...
from flask import Flask, jsonify, request, redirect, url_for, abort, g, render_template, make_response
...
def showCategories():
  categories = session.query(Category).all()
  return render_template('categories.html', categories=categories, isUserLoggedIn=isUserLoggedIn())

@app.route('/categories/<int:cat_id>/items')
def showCategory(cat_id):
  category = session.query(Category).filter_by(id=cat_id).one()
  items = session.query(Item).filter_by(cat_id=category.id).all()
  return render_template('category.html', category=category, items=items, item_num=len(items), isUserLoggedIn=isUserLoggedIn())

@app.route('/categories/<int:cat_id>/items/<int:item_id>')
def showItem(cat_id, item_id):
  category = session.query(Category).filter_by(id=cat_id).one()
  item = session.query(Item).filter_by(id=item_id).one()
  return render_template('item.html', category=category, item=item, isUserLoggedIn=isUserLoggedIn())
